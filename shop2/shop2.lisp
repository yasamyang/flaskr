...
(defun find-plans (problem
  ...
  (let* ((*start-run-time* (get-internal-run-time))
	...
	;;;***exc my action print***
	(action (mapcar #'shorter-plan *plans-found*))
    ))
...
;;;***my action print***
(defun action (x)
    (with-open-file (str "static/action" :direction :output :if-exists :overwrite)
        (format str "~s~%" x))     
    ;(setf str (open "action" :direction :output :if-exists :append))
	;(format str "~s~%" x)
	;(close str)
)
;;;******
